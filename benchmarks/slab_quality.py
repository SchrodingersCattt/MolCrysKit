...
try:
    ...
    from molcrys_kit.operations import generate_topological_slab, generate_slabs_with_terminations
...
def run_benchmark():
    ...
    try:
        ...
        # Generate termination-aware slabs (topological policy) and save them with metadata
        term_results = generate_slabs_with_terminations(
            structure_or_crystal=crystal,
            miller_index=MILLER,
            min_slab_size=None,           # fallback to 3 layers by default
            min_vacuum_size=VACUUM,
            center_slab=True,
            unique_terminations="topo",
            term_selection="all",
        )
        for slab, info in term_results:
            term_path = OUTPUT_DIR / f"molcrys_kit_slab_term_{info.termination_index:03d}.cif"
            # Write CIF with termination metadata header
            write_cif(slab, filename=str(term_path), metadata={"termination_info": info})
        print(f"Saved {len(term_results)} termination slabs to {OUTPUT_DIR}")
    except Exception as e:
