...
def _graph(graph: PeriodicGraph):
    return {
        ...
        "edges": [
            {
                "left_node": e.left_node,
                "right_node": e.right_node,
                "right_image_shift": list(e.right_image_shift),
                "rule_id": e.rule_id,
                "closure": e.closure,
                "left_port": e.left_port,
                "right_port": e.right_port,
            }
...
def _atom_records(atoms):
    ...
    if any(name not in atoms.arrays for name in names):
        raise ValueError(
            f"periodic bundle atoms must contain arrays: {', '.join(names)}"
        )
    return [
        {
            "symbol": atoms.symbols[index],
            **{
                name: atoms.arrays[name][index].item()
                if hasattr(atoms.arrays[name][index], "item")
                else atoms.arrays[name][index]
                for name in names
            },
        }
...
def _normalise_format(value: str | None):
    ...
    if key not in _FORMATS:
        raise ValueError(
            f"unsupported periodic bundle format {value!r}; expected one of {tuple(_FORMATS)}"
        )
    return key
...
def _output_path(output: str | Path, format_name: str | None):
    ...
    selected = (
        _normalise_format(format_name)
        if format_name is not None
        else (inferred or "cif")
    )
    if inferred is not None and format_name is not None and selected != inferred:
        raise ValueError(
            f"output suffix {path.suffix!r} conflicts with format {selected!r}"
        )
...
def write_periodic_bundle(
    bundle: PeriodicBundle,
    output: str | Path,
    *,
    ...
    if not overwrite and (structure.exists() or sidecar.exists()):
        raise FileExistsError(f"bundle output exists: {structure}")
    ...
    if format_name == "extxyz":
        files.update({"extxyz": structure.name, "extxyz_sha256": digest})
    payload = dict(bundle.metadata)
    payload.update(
        {
            "files": files,
            "atom_count": len(bundle.atoms),
            "atom_records": _atom_records(bundle.atoms),
            "instances": [
                {
                    "instance_id": i.instance_id,
                    "template_id": i.template_id,
                    "chain_id": i.chain_id,
                    "repeat_id": i.repeat_id,
                    "rotation": [list(row) for row in i.rotation],
                    "translation": list(i.translation),
                }
                for i in bundle.instances
            ],
            "periodic_graph": _graph(bundle.graph),
...
def read_periodic_bundle(structure: str | Path, sidecar: str | Path | None = None):
    ...
    sidecar_path = (
        Path(sidecar) if sidecar is not None else structure.with_suffix(".json")
    )
    payload = json.loads(sidecar_path.read_text(encoding="utf-8"))
    files = payload.get("files", {})
    format_name = _normalise_format(
        files.get("format") or _SUFFIXES.get(structure.suffix.lower()) or "extxyz"
    )
    atoms = ase.io.read(structure, format=_FORMATS[format_name], index=0)
    expected = files.get("structure_sha256") or files.get("extxyz_sha256")
    if expected and expected != _sha256(structure):
        raise ValueError(
            "periodic bundle checksum mismatch: structure changed after sidecar creation"
        )
    if payload.get("cell") is not None:
        atoms.set_cell(np.asarray(payload["cell"], dtype=float), scale_atoms=False)
    if payload.get("pbc") is not None:
        atoms.pbc = tuple(bool(value) for value in payload["pbc"])
    records = payload.get("atom_records", ())
    if records:
        if len(records) != len(atoms):
            raise ValueError(
                "periodic bundle atom_records do not match structure atom count"
            )
        expected_symbols = [record.get("symbol") for record in records]
        if (
            all(symbol is not None for symbol in expected_symbols)
            and list(atoms.get_chemical_symbols()) != expected_symbols
        ):
