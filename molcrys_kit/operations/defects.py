...
class VacancyGenerator:
    ...
    def find_removable_cluster_indices(self, target_spec: Optional[Dict[str, int]] = None, 
                                     seed_index: Optional[int] = None) -> List[int]:
        """
        ...
        for species_id, count in target_spec.items():
            ...
            if count > available_count:
                raise ValueError(f"Not enough molecules of type {species_id}. "
        ...
        if seed_index is not None:
            ...
            if seed_species_id is None or seed_species_id not in needed_spec:
                raise ValueError(f"Seed index {seed_index} does not belong to a requested species")
            ...
            rarest_species = min(
                (s_id for s_id in needed_spec.keys() 
                 if s_id in self.analyzer.species_map),
                key=lambda s_id: len([idx for idx in self.analyzer.species_map[s_id] 
                                    if idx not in removal_list])
            )
            
            # Find an available molecule of the rarest species
            available_indices = [idx for idx in self.analyzer.species_map[rarest_species] 
                               if idx not in removal_list]
            if not available_indices:
                raise ValueError(f"No available molecules of rarest species {rarest_species}")
        ...
        while needed_spec:
            ...
            for candidate_idx in candidate_indices:
                candidate_pos = self.crystal.molecules[candidate_idx].get_centroid_frac()
                
                # Find minimum distance to any molecule already in removal_list
                min_dist = float('inf')
                for removed_idx in removal_list:
                    removed_pos = self.crystal.molecules[removed_idx].get_centroid_frac()
                    dist = minimum_image_distance(removed_pos, candidate_pos, self.crystal.lattice)
                    if dist < min_dist:
                        min_dist = dist
    ...
    def generate_vacancy(self, target_spec: Optional[Dict[str, int]] = None, 
                        seed_index: Optional[int] = None, 
                        method: str = 'spatial_cluster') -> MolecularCrystal:
        """
        ...
        remaining_molecules = [
            mol.copy() for idx, mol in enumerate(self.crystal.molecules)
