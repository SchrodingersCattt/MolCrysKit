...
def compute_interatomic_distances(crystal: MolecularCrystal) -> List[float]:
    ...
    if not ASE_AVAILABLE:
        raise ImportError(
            "ASE is required for distance computation. Please install it with 'pip install ase'"
        )
...
def evaluate_steric_clash(
    crystal: MolecularCrystal, clash_threshold: float = 0.8
) -> float:
    """
