...
def main():
    ...
    # Wrap it in an EnhancedMolecule
    enhanced_water = EnhancedMolecule(water)
    ...
    centroid = enhanced_water.get_centroid()
    center_of_mass = enhanced_water.get_center_of_mass()
    ...
    # Get ellipsoid radii
    radii = enhanced_water.get_ellipsoid_radii()
    print(f"Ellipsoid radii: a={radii[0]:.6f}, b={radii[1]:.6f}, c={radii[2]:.6f}")
    
    # Get principal axes
    ax1, ax2, ax3 = enhanced_water.get_principal_axes()
    ...
    enhanced_methane = EnhancedMolecule(methane)
    ...
    # Get centroid and center of mass
    centroid = enhanced_methane.get_centroid()
    center_of_mass = enhanced_methane.get_center_of_mass()
    print(f"Centroid: [{centroid[0]:.6f}, {centroid[1]:.6f}, {centroid[2]:.6f}]")
    print(f"Center of mass: [{center_of_mass[0]:.6f}, {center_of_mass[1]:.6f}, {center_of_mass[2]:.6f}]")
    
    # Get ellipsoid radii
    radii = enhanced_methane.get_ellipsoid_radii()
    print(f"Ellipsoid radii: a={radii[0]:.6f}, b={radii[1]:.6f}, c={radii[2]:.6f}")
    
    # Get principal axes
    ax1, ax2, ax3 = enhanced_methane.get_principal_axes()
    print(f"Principal axes:")
    print(f"  Axis 1: [{ax1[0]:.6f}, {ax1[1]:.6f}, {ax1[2]:.6f}]")
    print(f"  Axis 2: [{ax2[0]:.6f}, {ax2[1]:.6f}, {ax2[2]:.6f}]")
    print(f"  Axis 3: [{ax3[0]:.6f}, {ax3[1]:.6f}, {ax3[2]:.6f}]")
