...
def main():
    ...
    if not os.path.exists(input_path):
        print(f"Input file {input_path} does not exist. Please provide the MAP structure.")
        return
    ...
    expected_methyl_groups = orig_counts.get('C', 0)  # Each C should form a methyl group with 3 H
    expected_ammonium_groups = orig_counts.get('N', 0)  # Each N should form an ammonium group with 3 H
    
    print(f"Expected methyl groups: {expected_methyl_groups}, Found: {len(methyl_carbons)}")
    print(f"Expected ammonium groups: {expected_ammonium_groups}, Found: {len(ammonium_nitrogens)}")
    ...
    if methyl_carbons:
                ...
                angle = np.degrees(
                    np.arccos(
                        np.clip(
                            ...
                            (np.linalg.norm(bond_vectors[i]) * np.linalg.norm(bond_vectors[j])),
                            -1.0, 1.0
                        )
                    )
                )
        ...
        print(f"Expected ~109.5° for tetrahedral arrangement")
    ...
    if ammonium_nitrogens:
        ...
        print(f"Expected ~109.5° for tetrahedral arrangement (with one position occupied by a lone pair)")
