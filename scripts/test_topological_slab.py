...
def main():
    ...
    try:
        ...
        print(f"  Lattice parameters: a={a:.3f}, b={b:.3f}, c={c:.3f}, α={alpha:.3f}, β={beta:.3f}, γ={gamma:.3f}")
        ...
        if slab_atoms == expected_atoms:
            print(f"✓ Atom count check passed: {slab_atoms} = {original_atoms} × 3 layers")
        else:
        ...
        if len(slab_molecules) == expected_molecule_count:
            print(f"✓ Molecule count check passed: {len(slab_molecules)} = {len(crystal.molecules)} × 3 layers")
        else:
            print(f"✗ Molecule count check failed: {len(slab_molecules)} ≠ {expected_molecule_count}")
