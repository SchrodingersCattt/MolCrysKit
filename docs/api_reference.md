...
- [interaction_profile](../molcrys_kit/analysis/interactions/profile.py): Run all weak-interaction detectors and summarize each interaction kind as `{count, max, mean, sum}` while retaining raw scored interaction records. H···H contacts are reported as `close_contact` because their score describes geometric close-contact character rather than a uniformly attractive interaction.
