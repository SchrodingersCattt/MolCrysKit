...
4. `operations._path_core` supplies the same λ, PBC-image, frame-copy, and
	SE(3)/SO(3)/SLERP primitives used by the ordinary and reactive adapters.
5. `interpolate_symmetry_path` generates only proper rigid interior motions and
...
When an explicit target is supplied, the planner first verifies that its mapped
coordinates match the endpoint generated by the affine operation within
