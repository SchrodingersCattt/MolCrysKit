...
def test_pap_m5_disorder():
    ...
    assert len(disorder_info.labels) == len(disorder_info.symbols), "Labels and symbols should have same length"
    assert len(disorder_info.labels) == len(disorder_info.occupancies), "Labels and occupancies should have same length"
    assert len(disorder_info.labels) == len(disorder_info.disorder_groups), "Labels and disorder groups should have same length"
    
    # Check that labels retain suffixes
    labels_with_suffixes = [label for label in disorder_info.labels if any(c.isalpha() and c != label[0] or c.isdigit() and label.index(c) > 0 for c in label[1:])]
...
def test_dap_4_disorder():
    ...
    non_zero_groups = [group for group in disorder_info.disorder_groups if group != 0]
    assert len(non_zero_groups) == 0, f"DAP-4.cif should have all disorder groups equal to 0, but found {len(non_zero_groups)} non-zero groups"
    ...
    assert len(low_occupancy_atoms) > 0, "DAP-4.cif should have atoms with occupancy < 1.0"
    ...
    approx_half_occupancy = [occ for occ in disorder_info.occupancies if 0.4 <= occ <= 0.6]
...
def test_general_functionality():
    ...
    assert isinstance(disorder_info.frac_coords, np.ndarray), "frac_coords should be a numpy array"
    assert disorder_info.frac_coords.shape[1] == 3, "frac_coords should have 3 columns for x, y, z"
    assert len(disorder_info.frac_coords) == len(disorder_info.labels), "frac_coords should have same number of rows as labels"
