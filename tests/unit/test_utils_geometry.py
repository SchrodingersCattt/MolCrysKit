...
def test_frac_to_cart():
    ...
    assert np.allclose(cart_coords, expected_cart), f"Expected {expected_cart}, got {cart_coords}"
...
def test_cart_to_frac():
    ...
    assert np.allclose(frac_coords, expected_frac), f"Expected {expected_frac}, got {frac_coords}"
...
def test_normalize_vector():
    ...
    assert np.allclose(normalized_zero, expected_zero), f"Expected {expected_zero}, got {normalized_zero}"
...
def test_minimum_image_distance():
    ...
    assert abs(min_dist - regular_dist) < 1e-10, f"Expected {regular_dist}, got {min_dist}"
    ...
    assert abs(min_dist2 - expected_dist) < 1e-10, f"Expected {expected_dist}, got {min_dist2}"
...
def test_rotate_vector():
    ...
    expected = np.array([0.0, 1.0, 0.0])  # Should point along y-axis after 90 deg rotation
    assert np.allclose(rotated, expected, atol=1e-10), f"Expected {expected}, got {rotated}"
    ...
    assert np.allclose(rotated2, expected2, atol=1e-10), f"Expected {expected2}, got {rotated2}"
...
def test_get_missing_vectors():
    ...
    assert len(vectors) == 2, f"Expected 2 vectors for linear with no neighbors, got {len(vectors)}"
    
    # Test linear geometry with one neighbor
    existing_neighbors = [np.array([1.0, 0.0, 0.0])]
    vectors = get_missing_vectors(center, existing_neighbors, "linear", 1.0)
    assert len(vectors) == 1, f"Expected 1 vector for linear with one neighbor, got {len(vectors)}"
    
    # Test tetrahedral geometry with one neighbor
    existing_neighbors = [np.array([1.0, 0.0, 0.0])]
    vectors = get_missing_vectors(center, existing_neighbors, "tetrahedral", 1.0)
    assert len(vectors) == 3, f"Expected 3 vectors for tetrahedral with one neighbor, got {len(vectors)}"
    ...
    assert len(vectors) == 1, f"Expected 1 vector for trigonal planar with two neighbors, got {len(vectors)}"
    
    # Test bent geometry with one neighbor
    existing_neighbors = [np.array([1.0, 0.0, 0.0])]
    vectors = get_missing_vectors(center, existing_neighbors, "bent", 1.0)
    assert len(vectors) == 1, f"Expected 1 vector for bent with one neighbor, got {len(vectors)}"
