...
def test_simple_molecular_system():
    ...
    assert len(molecules_sorted[0]) == 1, f"Expected first molecule to have 1 atom, got {len(molecules_sorted[0])}"
    assert molecules_sorted[0].get_chemical_symbols()[0] == 'Cl'
    
    # Second molecule should be the H-O molecule (2 atoms)
    assert len(molecules_sorted[1]) == 2, f"Expected second molecule to have 2 atoms, got {len(molecules_sorted[1])}"
    ...
    assert len(graph.nodes()) == 2, f"Expected 2 nodes in graph, got {len(graph.nodes())}"
    assert len(graph.edges()) == 1, f"Expected 1 edge in graph, got {len(graph.edges())}"
    
    # Check that the isolated Cl atom has no bonds
    graph_cl = molecules_sorted[0].graph
    assert len(graph_cl.nodes()) == 1, f"Expected 1 node in Cl graph, got {len(graph_cl.nodes())}"
    assert len(graph_cl.edges()) == 0, f"Expected 0 edges in Cl graph, got {len(graph_cl.edges())}"
...
def test_more_complex_system():
    ...
    assert molecule_sizes == [1, 3, 4], f"Expected molecule sizes [1, 3, 4], got {molecule_sizes}"
...
def run_tests():
    """Run all tests."""
    tests = [
        test_simple_molecular_system,
        test_more_complex_system
    ]
